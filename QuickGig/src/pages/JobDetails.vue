...
const fetchHelperCount = async () => {
  ...
  try {
    // Query the chats table to count unique job seekers for this job
    const { data, error, count } = await supabase
      .from('chats')
      .select('job_seeker_id', { count: 'exact', head: false })
